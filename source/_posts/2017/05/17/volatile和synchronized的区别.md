...
内存可见控制的是线程执行结果在内存中对其它线程的可见性。根据{% post_link 全面理解java内存模型 Java内存模型 %}的实现，线程在具体执行时，会先拷贝主存数据到线程本地（CPU缓存），操作完成后再把结果从线程本地刷到主存。
...
如果想更好地理解这两个关键字的作用，强烈建议看一下这篇文章：{% post_link 全面理解java内存模型 Java内存模型 %}

{% blockquote @原文地址 https://blog.csdn.net/suifeng3051/article/details/52611233 %}
{% endblockquote %}